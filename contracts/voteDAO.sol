...
contract AdvancedDAO {
    ...
    modifier onlyTokenHolders() {
        require(governanceToken.balanceOf(msg.sender) > 0, "Only token holders can perform this action");
        _;
    }
    ...
    function vote(uint256 _proposalId, bool _support) external onlyTokenHolders {
        ...
        if (_support) {
            proposal.voteCount = proposal.voteCount.add(governanceToken.balanceOf(msg.sender));
        }
    ...
    function executeProposal(uint256 _proposalId) external onlyOwner {
        ...
        uint256 quorumVotes = totalSupply.mul(quorumPercentage).div(100);
